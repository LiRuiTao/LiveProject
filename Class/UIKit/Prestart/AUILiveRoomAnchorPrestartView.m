...
- (instancetype)initWithFrame:(CGRect)frame withModel:(nonnull AUIRoomLiveInfoModel *)model {
    ...
    if (self) {
        ...
        if (model.cover.length > 0) {
            [cover sd_setImageWithURL:[NSURL URLWithString:model.cover] placeholderImage:AUIRoomGetCommonImage(@"ic_list_bg")];
        }
        else {
            cover.image = AUIRoomGetCommonImage(@"ic_list_bg");
        }
