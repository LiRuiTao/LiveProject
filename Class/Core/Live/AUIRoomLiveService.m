...
- (void)sendData:(id<AUIRoomMessageDataModel>)data type:(AUIRoomMessageType)type uids:(NSArray<NSString *> *)uids skipMuteCheck:(BOOL)skipMuteCheck skipAudit:(BOOL)skipAudit completed:(void (^)(BOOL))completed {
...
- (void)onMuteGroup:(AUIRoomMessageModel *)message {
    ...
    if (self.onReceivedMuteAll) {
        self.onReceivedMuteAll(message.sender, self.isMuteAll);
    }
}
